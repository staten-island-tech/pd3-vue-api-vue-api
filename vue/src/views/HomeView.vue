<template>
  <div>
    ...
    <div class="container">
      <Card v-for="(call, index) in filteredCalls" :key="call.unique_key" :id="index + 1" :request="call"> </Card>
...
const hasNext = computed(() => {
  ...
  filteredCalls.value = calls.value.filter(call => {
    return call.incident_address && call.incident_address.toLowerCase().includes(searchTerm.toLowerCase()) ||
           call.borough && call.borough.toLowerCase().includes(searchTerm.toLowerCase()) ||
           call.complaint_type && call.complaint_type.toLowerCase().includes(searchTerm.toLowerCase()) ||
